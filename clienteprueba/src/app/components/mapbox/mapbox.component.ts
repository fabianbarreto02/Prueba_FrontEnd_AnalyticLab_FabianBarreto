...
export class MapboxComponent implements OnInit {
  // Varible para cargar la capa  del mapa y el servicio que carga la layer del api
  datasource: any;
  ...
  ngOnInit() {
    // AccessToken para el map
    Mapboxgl.accessToken = environment.mapboxkey;
    // Definicion de parametros iniciales del map
    const mapa = new Mapboxgl.Map({
      container: 'mapa-mapbox', // container id
      style: 'mapbox://styles/mapbox/streets-v11',
      center: [-74.08699035644531, 4.742043976363009], // LNG,LAT
      zoom: 9.5 // starting zoom
    });

    // Carga layer proveniente del api
    mapa.on('load', () => {
      ...
      // Adiciona la capa de los puntos dados
      mapa.addLayer({
        id: 'points_data',
        source: 'pointsSource',
        type: 'circle',
    ...
    // Muestra los popup en el map con un click en los puntos
    mapa.on('click', e => {
      ...
      if (result.length) {
        ...
        if (String(id) === 'points_data') {
          popup
            .setLngLat(e.lngLat)
            .setHTML(
              `<h6 style='text-align:center'>Id: ${id_data}<br>NIT: ${nit}<br>Nombre: ${content_name}
  ...
  // Metodo que carga layer del api dado
  loadData() {
